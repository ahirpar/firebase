...
//        Ingredients Add Start
                ...
                if (eid == '') {
                    ...
                    var firebaseRefedit = firebase.database().ref('items/' + eid + '/Ingredients/' + orderId + '/');
                    firebaseRefedit.set({
                        id: lastId,
                        ingredientsName: addIngredientsName.trim(),
                        order: orderId,
                ...
                if (eid == '') {
                    ...
                    var firebaseRefedit = firebase.database().ref('items/' + eid + '/Extra/' + orderId + '/');
                    firebaseRefedit.set({
                        id: lastId,
...
//    return false
    ...
    if (eid == '') {
        ...
        firebaseRefedit.orderByChild("itemId").limitToLast(1).on('value', function (snapshot) {
            snapshot.forEach(function (childSnapshot) {
                lastId = childSnapshot.val().itemId;
            });
        });
        ...
        alert(lastId);
        return false;
        var mountainImagesRef = storageRef.child('images/items/item' + lastId + '.jpg')
//        items.orderByChild("itemId").limitToLast(1).on('value', function (snapshot) {
//            snapshot.forEach(function (childSnapshot) {
//                lastId = childSnapshot.val().itemId;
//            });
