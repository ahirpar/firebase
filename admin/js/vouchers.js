    ...
    $('#vouchers-form').validate({// initialize plugin
        ...
        submitHandler: function (form) {
            ...
            if (Code != '' && MaxUses != '' && ValidFrom != '' && ValidTo != '') {
                var lastId = "";
                Vouchers.orderByKey().limitToLast(1).on('value', function (snapshot) {
...
Vouchers.orderByKey().limitToFirst(pageSize).on('value', function (snapshot) {
...
Vouchers.orderByKey().on('value', function (snapshot) {
    var pagination = "";
    $(".pagination").empty();
    var i = 0;
    var j = 0;
    ...
    for (var k = 1; k <= j; k++) {
        ...
        pagination += '<li class="' + active + '"><a href="javascript:;" onclick=\'return getpagedata("' + k + '","' + pageSize + '")\'>' + k + '</a></li>';
    }
//    $(".pagination").append('<li><a href="#" aria-label="Previous"><span aria-hidden="true" class="mdi mdi-chevron-left"></span></a></li>');
    $(".pagination").append(pagination);
//    $(".pagination").append('<li><a href="#" aria-label="Next"><span aria-hidden="true" class="mdi mdi-chevron-right"></span></a></li>');
});



function getpagedata(pageNumber, pageSize) {
    var start = Number(pageNumber) * Number(pageSize);
    var pageSize = Number(pageSize);
    Vouchers.orderByChild("voucherId").startAt(start).limitToFirst(pageSize).on('value', function (snapshot) {
        ...
        $("#getcount").append("Showing " + pageNumber + " to " + pageNumber * pageSize + " of " + i + " entries");
    });
}
